...
# این اسکریپت باید به صورت خودکار در زمان راه‌اندازی Replit اجرا شود

echo "شروع راه‌اندازی خودکار ربات تلگرام در تاریخ $(date)" > startup.log
echo "----------------------------------------" >> startup.log

# بررسی وجود فایل‌های لازم
if [ ! -f "ten_minute_scheduler.py" ]; then
    echo "خطا: فایل ten_minute_scheduler.py یافت نشد!" >> startup.log
    exit 1
fi

if [ ! -f "enhanced_telegram_reporter.py" ]; then
    echo "خطا: فایل enhanced_telegram_reporter.py یافت نشد!" >> startup.log
    exit 1
fi

# توقف هر نمونه در حال اجرا
echo "در حال توقف نمونه‌های قبلی..." >> startup.log
pids=$(ps aux | grep "ten_minute_scheduler.py" | grep -v grep | awk '{print $2}')
if [ -n "$pids" ]; then
    for p in $pids; do
        echo "توقف فرآیند $p..." >> startup.log
        kill -9 $p 2>/dev/null
    done
    echo "فرآیندهای قبلی متوقف شدند." >> startup.log
else
    echo "هیچ فرآیند قبلی یافت نشد." >> startup.log
fi

# حذف فایل‌های PID قدیمی
rm -f ten_minute_scheduler.pid 2>/dev/null

echo "در حال راه‌اندازی ربات ارسال گزارش..." >> startup.log
...
echo "ربات با شناسه فرآیند $(cat ten_minute_scheduler.pid) راه‌اندازی شد." >> startup.log
echo "برای مشاهده وضعیت: tail -f ten_minute_scheduler.log" >> startup.log
echo "پایان راه‌اندازی خودکار در تاریخ $(date)" >> startup.log
echo "----------------------------------------" >> startup.log

# ارسال یک پیام تایید به تلگرام
echo "در حال ارسال پیام اطلاع‌رسانی به تلگرام..." >> startup.log
python -c "
import os
import sys
